...
from classifier_model.Models.Transformer import Transformer
...
class PatchEmbeddings(nn.Module): # 将图像块划分为patches，并映射到隐藏空间, 同时返回原始patches，用于MAE重构
    ...
    def forward(self, x):
        x = self._make_channels_divisible(x, self.split_size) # 保证通道数是split_size的整数倍
...
class SpecTransformer(nn.Module):
    def __init__(self, img_size, config=get_base_configs(), global_pool=False):
        ...
        if global_pool: # 注意在源码中，预训练时不要使用global_pool，因为它会删除encoder_norm
            print("Use global pool for SpecTransformer")
            self.fc_norm = nn.LayerNorm(config['hidden_size'], eps=1e-6)
            del self.transformer.encoder_norm # 删除原有的layer norm
    ...
    def forward(self, x):
        ...
        if self.global_pool:
            x = self.fc_norm(x[:, 1:]).mean(dim=1)  # 全局平均池化，去掉cls token
            return x
        else: return x[:, 0]  # 直接返回CLS token的编码