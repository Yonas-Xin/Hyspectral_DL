...
def train(frame, model, optimizer, train_dataloader, eval_dataloader=None, scheduler=None, 
    ...
    IF_DRAW_FEATURE_MAPS = model.if_draw_feature_map()
    ...
    if SWANLAB_AVAILABLE:
        ...
        matrics = swanlab.confusion_matrix(frame.best_all_labels, frame.best_all_preds, class_names=range(np.max(frame.best_all_labels)+1)) # 生成热力图，并上传到swanlab
        swanlab.log({"confusion_matrix_custom": matrics})
    frame.logger.info(best_result)
    log_report(frame.best_all_labels.tolist(), frame.best_all_preds.tolist(), log_writer=frame.logger)

def log_report(all_labels, all_preds, log_writer=None):
    accuracy = accuracy_score(all_labels, all_preds)
    clf_report = classification_report(all_labels, all_preds, digits=4)
    conf_matrix = confusion_matrix(all_labels, all_preds)
    kappa = cohen_kappa_score(all_labels, all_preds)

    log_writer.info(f"Overall Accuracy: {accuracy:.4f}. Cohen's Kappa: {kappa:.4f}.")
    log_writer.info(f"Classification Report:")
    log_writer.info(clf_report + "\n")
    log_writer.info("Confusion Matrix:")
    log_writer.info(np.array2string(conf_matrix, separator=', '))